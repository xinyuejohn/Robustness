...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        class_samples = self._sample_noise_predictions(inputs, n0, batch_size)
        
        top_class = class_samples.argmax().item()
        
        robustness_samples = self._sample_noise_predictions(inputs, num_samples, batch_size)
        
        nA = robustness_samples[top_class].item()
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, batch_size: int) -> int:
        ...
        """
        self.base_classifier.eval()
        class_counts = self._sample_noise_predictions(inputs, num_samples, batch_size).cpu() # TODO
        
        # https://arxiv.org/abs/1902.02918

        first, second = class_counts.argsort()[::-1][:2]
        nA = class_counts[first]
        nB = class_counts[second]
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                # shape inputs [1, 1, 28, 28]
                # shape [100, 1, 28, 28]
                # b = torch.cat([inputs for _ in range(this_batch_size)])
                b = torch.cat(this_batch_size * [inputs])
                
                # shape [100, 1, 28, 28]
                noise = torch.randn_like(b) * self.sigma
                
                # to resolve min max error
                # shape [100, 1, 28, 28]
                batch_noise = torch.clamp(b + noise, 0, 1).to(self.device())
                
                # shape [100]
                preds = torch.argmax(self.base_classifier(batch_noise), 1).to(self.device())
                

                counts = torch.zeros(self.num_classes, dtype=torch.long).to(self.device())
                for index in preds.cpu().numpy(): #.cpu()
                    counts[index] += 1
